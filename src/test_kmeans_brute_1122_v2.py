...
aggregated_train = long_train.groupby(["sno", "hour", "minute"]).agg(
    mean_20=("sbi", "mean"), std_20=("sbi", "std")
)
aggregated_train = (
    aggregated_train.reset_index()
    .merge(
        long_train.groupby("sno").agg(mean_sta=("sbi", "mean"), std_sta=("sbi", "std")),
        on="sno",
    )
    .set_index(["sno", "hour", "minute"])
)
...
# to_group_df's index is (sno, hour, minute) a.k.a. input
# to_group_df's columns are (tot, mean_20, std_20, mean_std, std_sta) a.k.a. properties

property_names = ["mean_20", "std_20", "mean_sta", "std_sta"]
...
MAE 4.995216836734694
Score 0.34451564506732707
"""
...
out_df.to_csv("public_test_submission_1122_0121.csv", index=False)
