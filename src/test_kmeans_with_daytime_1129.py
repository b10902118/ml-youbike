...
def is_daytime(s):
    return (s.dt.hour >= 8) & (s.dt.hour < 17)
...
def prepare_data(train):
    ...
    long_train["is_daytime"] = is_daytime(long_train["time"])
    ...
    tmp_df = pd.DataFrame(
        {
            ...
            "is_daytime": is_daytime(time_range),
        }
...
# km = make_pipeline(StandardScaler(), KMeans(n_clusters=n_clusters, n_init="auto"))
...
test_pred = get_prediction(km, test.index, group_df, aggregated_train)
evaluation(
...
kmeans 5000 (~11/14) + q25, q50, q75

MAE 4.760561342592593
Score 0.33006783742805573
...
kmeans 2000 (~11/14) + q25, q50, q75, minute_of_day + standard scaler

MAE 4.800264550264551
Score 0.33461716094694244
"""
...
out_df.to_csv("public_test_submission_1129_1943.csv", index=False)
