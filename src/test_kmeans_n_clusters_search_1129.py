...
def prepare_data(train):
    ...
    to_group_df.set_index(index_names, inplace=True)
    ...
    aggregated_train = long_train.groupby(index_names).agg(
        mean_20=("sbi", "mean"),
        std_20=("sbi", "std"),
    )
    aggregated_train = (
...
def get_group_assignment_df(kmeans, time_range):
...
def do_train(kmeans, train, long_train, n_clusters):
    # find best sbi for each group
    t_df = get_group_assignment_df(kmeans, train.index)
...
def get_prediction(kmeans, time_range, group_df):
    tmp_df = get_group_assignment_df(kmeans, time_range)
...
kmeans 8000 (~11/14) + q25, q50, q75

MAE 4.762276785714286
Score 0.3300698831568075
"""
...
public_test_predict = get_prediction(km, public_test_range, group_df)
private_test_range = pd.date_range("2023/12/04 00:00", "2023/12/10 23:59", freq="20min")
private_test_predict = get_prediction(km, private_test_range, group_df)
...
out_df.to_csv("public_test_submission_1129_1833.csv", index=False)
