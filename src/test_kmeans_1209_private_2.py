...
SMALL_CACHE_DIR = Path("./small_cache")
...
def load_all_data(demographics):
    with ProcessPoolExecutor(max_workers=8) as executor:
        df_over_dates = []
        for date_dir in sorted(list(RELEASE_DIR.iterdir())):
            date = date_dir.name
            small_cache_path = SMALL_CACHE_DIR / f"{date}.pkl"
            if small_cache_path.exists():
                df = pd.read_pickle(small_cache_path)
                print(f"Loading {date} from small cache")
            else:
                files = [
                    file
...
yydf = get_group_assignment_df(km, test.index, aggregated_train)
yydf = yydf.reset_index().merge(group_df, how="left", on="group")
yydf = yydf.merge(test_true, on=["sno", "time"]).sort_values(by=["sno", "time"])
yydf["terr"] = np.abs(yydf["sbi"] - yydf["best_sbi"])

for grp in group_df.index:
    yy = yydf[yydf["group"] == grp]
    group_df.loc[grp, "test_err"] = error(
        yy["sbi"].values, yy["best_sbi"].values, yy["tot"].values
    )
print(group_df.sort_values(by="test_err", ascending=False))

print(
    yydf[yydf["group"] == 1140][
        ["sno", "time", "is_holiday", "tot", "best_sbi", "sbi", "terr"] + property_names
    ].to_string()
)
...
private_test_range = pd.date_range("2023/12/11 00:00", "2023/12/17 23:59", freq="20min")
...
out_df.to_csv("private_2_submission_1209_0943.csv", index=False)
