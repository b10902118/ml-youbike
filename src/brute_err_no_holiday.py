...
def brute(
    y_true: np.ndarray, tot: int, step: np.float64 = 0.1
...
result_df = pd.DataFrame(
    columns=ntu_snos,
    index={
        "time": pd.date_range("00:00", "23:59", freq="20min").time,
        "holiday": [True, False],
    },
    dtype=np.float64,
)
...
for sno, tot in zip(ntu_snos, ntu_tots):
    ...
    # print(sd)
    # exit()
    psd = pd.pivot_table(sd, index="date", columns="time", values="sbi")
    # sno col have its sbi
    # print(psd)
    for t in psd.columns:
        # print(t, sno)
        sbi, err = brute(y_true=psd[t].values, tot=tot, step=0.1)  # step not matter
        Ein += err
        # print(f"{t} sbi:{sbi}   err: {err}")
        result_df.at[t, sno] = np.float64(sbi)
